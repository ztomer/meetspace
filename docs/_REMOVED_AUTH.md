...
6. **Subtree Squashing Quirks (Vendor directories)** — Git interactive rebasing of squashed subtree vendor dependencies (such as `./vendor/async-openai/` and `./vendor/gbnf-validator/`) merges them into the repository root, creating root-level file conflicts.
   - *Resolution*: Checkout the clean workspace root configurations (`.gitignore`, `Cargo.toml`, etc.) from your target rebase head, and restore the correct `./vendor/` directories using `git checkout <pre-rebase-backup-commit> -- vendor/`.
