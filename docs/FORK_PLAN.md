...
| 4 | **Google Calendar sign-in** — OAuth 2.0 PKCE with localhost redirect. Tauri opens browser to Google's OAuth URL, hosts a temporary localhost server to catch the `?code=…` redirect, exchanges via PKCE (no client secret needed). User does a one-time setup of their own Google Cloud OAuth client ID (free, no review needed for desktop). Tokens stored in OS keychain. | ⏳ Pending |
| 5 | **Outlook Calendar sign-in** — same pattern as #4 but against Microsoft Entra `/common/oauth2/v2.0/authorize`. User registers a public client in Azure portal, no secret. Microsoft Graph `/me/events` for fetch. | ⏳ Pending |
