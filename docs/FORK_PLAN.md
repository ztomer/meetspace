...
| 4 | **Google Calendar sign-in** — OAuth 2.0 PKCE with localhost redirect. | 🟡 Partial — sign-in flow done end-to-end. New `plugins/oauth/` Rust plugin (generic PKCE: random verifier → S256 challenge → axum listener on `127.0.0.1:0` → opens browser via `tauri_plugin_opener2` → catches `?code=…` → token-exchange POST → returns access/refresh tokens). Settings → Integrations → Google Calendar UI for entering client id + signing in + disconnect. Tokens persisted via tinybase (`google_client_id` / `google_access_token` / `google_refresh_token` / `google_token_expires_at`). **Still pending (8.4b):** actually USE the tokens to fetch calendar list / events — the existing `crates/calendar/src/fetch.rs` still calls the (deleted) upstream API. Need to replace with direct `https://www.googleapis.com/calendar/v3/...` calls and a TS-side refresh helper that auto-swaps refresh_token for fresh access_token on 401. |
| 5 | **Outlook Calendar sign-in** — same pattern against Microsoft Entra. | 🟡 Partial — sign-in flow done (shares `plugins/oauth/`). Settings → Integrations → Outlook Calendar UI. Tokens persisted (`outlook_client_id` / `outlook_access_token` / `outlook_refresh_token` / `outlook_token_expires_at`). **Still pending (8.5b):** wire MS Graph `/me/events` fetch through the new tokens, same as 8.4b. |
