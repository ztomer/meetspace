...
| 4 | **Dark mode** via a **centralized theming layer** — *don't* sprinkle `dark:` conditionals across 100+ files. Build a semantic-token layer first: CSS variables on `:root` + `[data-theme=dark]`, surfaced through Tailwind as `bg-surface`, `text-fg`, `border-subtle`, `bg-accent`, etc. Migrate components off raw `neutral-*` / `stone-*` scales onto the tokens. Toggle in Settings → App (system / light / dark, persisted via tinybase, honors `prefers-color-scheme` when system). One place to change a color and it propagates everywhere. | ⏳ Pending |
