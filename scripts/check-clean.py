...
# Git markers: `<<<<<<< name`, `>>>>>>> name`, `||||||| name` (7 chars + space or
# EOL) and the bare `=======` separator (exactly 7, anchored to avoid matching
# markdown/ascii rulers). The old regex required a trailing space and omitted
# `=======`, so it missed bare markers in generated files during a bad rebase.
...
def main():
    ...
    if leaks:
        print(
            "FAIL: upstream identifiers found (run: python3 scripts/rebrand_sweep.py):"
        )
        for p in leaks:
            print(f"  {p}")
        fail = 1
