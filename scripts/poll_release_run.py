...
def fetch_cask_version():
    url = "https://raw.githubusercontent.com/ztomer/homebrew-tap/main/Casks/meetspace.rb"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req) as response:
...
def main():
    ...
    print_info(f"Please open your browser and create a new Release for tag '{tag_name}' at:")
    ...
    while True:
        ...
        for run in runs:
            ...
            is_match = (
                (run.get("event") == "release" and run.get("head_branch") == tag_name) or
                (run.get("head_branch") == tag_name) or
                (run.get("event") == "release" and run.get("head_sha") == head_sha)
            )
            if is_match:
                run_id = run.get("id")
        ...
        print_info(f"Still waiting... ({elapsed}s elapsed). Please publish the GitHub release to start the build.")
        time.sleep(20)
    ...
    while True:
        ...
        if status == "completed":
            if conclusion == "success":
                ...
                print_err(f"GitHub Actions release build failed with conclusion: {conclusion}")
                sys.exit(1)
    ...
    while True:
        ...
        if current_version == expected_cask_version:
            print_ok(f"Homebrew Tap Cask successfully updated to version: {current_version}!")
            break
            
        elapsed = int(time.time() - start_time)
        if elapsed > 300: # 5 minutes timeout
            print_wrn(f"Timeout waiting for Homebrew update. Current version is still: {current_version}")
            print_info("You can verify the Cask manually at: https://github.com/ztomer/homebrew-tap")
            break
            
        print_info(f"Checking Cask version... (current: '{current_version}', expected: '{expected_cask_version}')")
        time.sleep(20)
