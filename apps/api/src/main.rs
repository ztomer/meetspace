...
async fn app() -> Router {
    ...
    let auth_state_pro = AuthState::new(&env.supabase.supabase_url)
        .with_required_entitlements(vec!["meetspace_pro".into(), "meetspace_lite".into()]);
    ...
    let cactus_config = meetspace_api_cactus::CactusProxyConfig {
    ...
    let auth_state_integration =
        AuthState::new(&env.supabase.supabase_url).with_required_entitlement("meetspace_pro");
    ...
    Router::new()
        ...
        .nest("/cactus", meetspace_api_cactus::router(cactus_config))
